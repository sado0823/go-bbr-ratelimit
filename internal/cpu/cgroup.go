...
import (
	"bufio"
	"fmt"
	"go-bbr-ratelimit/internal/util"
	"os"
	"path"
	"strconv"
	"strings"
...
func (c *cgroup) CpuAcctAllUsage() (uint64, error) {
	text, err := util.ReadText(path.Join(c.cgroupSet["cpuacct"], "cpuacct.usage")) // ex: 1838571117964229
	if err != nil {
		return 0, err
	}
	return util.ParseUint(text)
}

func (c *cgroup) CpuAcctPerUsage() ([]uint64, error) {
	text, err := util.ReadText(path.Join(c.cgroupSet["cpuacct"], "cpuacct.usage_percpu")) // ex: 924274335970490 914283397203835
	if err != nil {
		return nil, err
	}
...
func (c *cgroup) CpuPeriodUS() (uint64, error) {
	data, err := util.ReadText(path.Join(c.cgroupSet["cpu"], "cpu.cfs_period_us")) // ex: 100000
	if err != nil {
		return 0, err
	}

	return util.ParseUint(data)
...
func (c *cgroup) CpuQuotaUS() (int64, error) {
	data, err := util.ReadText(path.Join(c.cgroupSet["cpu"], "cpu.cfs_quota_us")) // ex: -1
	if err != nil {
		return 0, err
	}
...
func (c *cgroup) Cpus() ([]uint64, error) {
	text, err := util.ReadText(path.Join(c.cgroupSet["cpuset"], "cpuset.cpus")) // ex: 0-1
	if err != nil {
		return nil, err
	}
	return util.ParseUints(text)
}
