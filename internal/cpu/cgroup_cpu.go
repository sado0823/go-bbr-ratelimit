...
func systemCpuUsage() (uint64, error) {
	lines, err := util.ReadLines("/proc/stat")
	if err != nil {
		return 0, err
	}
...
func cpuMaxFreq() uint64 {
	feq := cpuFreq()
	data, err := util.ReadFile("/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq")
	if err != nil {
		return feq
	}
	// override the max freq from /proc/cpuinfo
	cfeq, err := util.ParseUint(data)
	if err == nil {
		feq = cfeq
	}
	return feq
...
func cpuFreq() uint64 {
	lines, err := util.ReadLines("/proc/cpuinfo")
	if err != nil {
		return 0
	}
	for _, line := range lines {
